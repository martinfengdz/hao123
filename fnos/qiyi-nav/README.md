# 奇易智能导航 · 飞牛 fnOS 应用包（FPK）V3.1.2 纯导航版
...
3. 产出 `qiyi-nav.fpk`，重命名为 `qiyi-nav_3.1.2.fpk` 即可上传飞牛。
...
- **默认飞牛本地离线构建**：后端源码与 Dockerfile 已内置进 FPK 的 `app/build/`，compose 默认 `build:` 从本地上下文构建，镜像名为 `qiyi-nav:3.1.2`。**不需要 GitHub、不需要推镜像、没有占位符**，飞牛能联网拉取 `node:18-alpine` 基础镜像即可安装。
- 想用 GHCR 在线镜像（多设备统一版本）：编辑 `app/docker/docker-compose.yaml`，删掉 `build:` 段、取消 `image: ghcr.io/<用户名>/qiyi-nav:3.1.2` 注释（用户名必须全小写），前提是已在 GitHub Public 仓库 `push` + `git tag v3.1.2` 触发 Actions 构建出该镜像。
...
## 飞牛端安装步骤
1. 应用中心 → 设置 → 手动安装应用 → 选 `qiyi-nav_3.1.2.fpk`
2. 向导直接点「下一步」（无必填项），确认
